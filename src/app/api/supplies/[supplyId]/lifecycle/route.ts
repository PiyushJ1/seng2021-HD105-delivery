...
export async function PATCH(req: NextRequest, context: any) {
  try {
    ...
    if (!newState || !ALLOWED_STATES.includes(newState)) {
      return NextResponse.json({ error: "Invalid or missing newState" }, { status: 400 });
    }
    ...
    if (!existingSupply) {
      return NextResponse.json({ error: "Supply ID not found" }, { status: 404 });
    }

    if (existingSupply.version !== expectedVersion) {
      return NextResponse.json({ error: "Version mismatch: expectedVersion does not match server" }, { status: 409 });
    }

    if (newState === "RECEIVED" && !existingSupply.receiptAdviceId) {
      return NextResponse.json({ error: "Cannot move to RECEIVED if no receipt advice exists" }, { status: 422 });
    }
