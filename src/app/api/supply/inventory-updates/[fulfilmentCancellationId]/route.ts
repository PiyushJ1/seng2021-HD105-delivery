...
export async function PUT(
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      const cancellation = await cancellationColl.findOne({
        fulfilmentCancellationId,
      });
      if (!cancellation)
        throw { status: 404, message: "Cancellation ID not found" };

      if (cancellation.applied)
        throw { status: 409, message: "Cancellation already applied" };
      ...
      for (const line of inventoryAdjustmentLines) {
        if (line.quantity < 0)
          throw { status: 400, message: "Negative quantity not allowed" };

        const cancellationItem = cancellation.items?.find(
          (i: any) => i.sku === line.sku,
        );
        if (!cancellationItem)
          throw { status: 422, message: `SKU ${line.sku} not in cancellation` };
        if (line.quantity > cancellationItem.quantity)
          throw { status: 422, message: "Quantity exceeds cancellable amount" };
        ...
        if (!result)
          throw {
            status: 404,
            message: `Inventory position for ${line.sku} not found`,
          };
