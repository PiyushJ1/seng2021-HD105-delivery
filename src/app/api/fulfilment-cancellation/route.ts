...
export async function POST(req: NextRequest) {
  ...
  const { despatchAdviceId, cancellationDate, cancelledItems, reason } =
    body as {
      despatchAdviceId?: string;
  ...
  const despatchAdvice = (await db
    .collection<DespatchAdviceDocument>("despatchAdvices")
    .findOne({ despatchAdviceId })) as DespatchAdviceDocument | null;
  ...
  (despatchAdvice.items || []).forEach((item) => {
