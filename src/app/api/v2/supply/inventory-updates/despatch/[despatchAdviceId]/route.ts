...
export async function PUT(
  ...
  for (const line of inventoryAdjustmentLines) {
    ...
    if (line.quantityDespatched < 0) {
      return NextResponse.json(
        { error: "negative quantityDespatched" },
        { status: 400 },
      );
    }
  }
  ...
  const despatch = await db
    .collection("despatch_advice")
    .findOne({ despatchAdviceId });
  if (!despatch)
    return NextResponse.json(
      { error: "despatch advice ID, warehouse ID or bin ID not found" },
      { status: 404 },
    );

  if (despatch.applied || despatch.inventoryUpdateApplied)
    return NextResponse.json(
      { error: "despatch already applied" },
      { status: 409 },
    );
  ...
  for (const item of despatch.items ?? []) {
    ...
    itemsBySku.set(item.sku, {
      totalQty:
        (existing?.totalQty ?? 0) +
        (item.quantityDespatched || item.quantity || 0),
  ...
  for (const line of inventoryAdjustmentLines) {
    ...
    if (!match)
      return NextResponse.json(
        { error: "SKU not present in despatch advice" },
        { status: 422 },
      );
    ...
    if (line.quantityDespatched > match.totalQty)
      return NextResponse.json(
        { error: "despatched quantity exceeds available/onHand" },
        { status: 422 },
      );
  }
  ...
  for (const line of inventoryAdjustmentLines) {
    ...
    const newAvailable =
      (existingRow?.available ?? 0) - line.quantityDespatched;
