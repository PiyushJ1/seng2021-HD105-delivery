...
export async function PUT(
  ...
  for (const line of inventoryAdjustmentLines) {
    ...
    if (line.quantityCancelled < 0) {
      return NextResponse.json({ error: "negative quantityCancelled" }, { status: 400 });
    }
  }
  ...
  const fc = await db.collection("fulfilmentCancellations").findOne({ fulfilmentCancellationId });
  if (!fc) return NextResponse.json({ error: "fulfilmentCancellationId, warehouseId or binId not found" }, { status: 404 });
  if (fc.inventoryUpdateApplied || fc.applied) return NextResponse.json({ error: "cancellation already applied" }, { status: 409 });
  ...
  if (!warehouse || !bin) return NextResponse.json({ error: "fulfilmentCancellationId, warehouseId or binId not found" }, { status: 404 });
  ...
  for (const item of (fc.items ?? [])) {
    ...
    itemsBySku.set(item.sku, {
      totalQty: (existing?.totalQty ?? 0) + (item.quantityCancelled || item.quantity || 0),
  ...
  for (const line of inventoryAdjustmentLines) {
    ...
    if (!match) return NextResponse.json({ error: "SKU not present in fulfilment cancellation" }, { status: 422 });
    if (line.uom && match.uom && line.uom !== match.uom) return NextResponse.json({ error: "invalid SKU or UoM mismatch" }, { status: 422 });
    if (line.quantityCancelled > match.totalQty) return NextResponse.json({ error: "cancelled quantity exceeds cancellable quantity" }, { status: 422 });
  }
  ...
  await db.collection("fulfilmentCancellations").updateOne({ fulfilmentCancellationId }, { $set: { applied: true, inventoryUpdateApplied: true, appliedAt } });

  return NextResponse.json({ fulfilmentCancellationId, applied: true, appliedAt, positionsUpdated });
}